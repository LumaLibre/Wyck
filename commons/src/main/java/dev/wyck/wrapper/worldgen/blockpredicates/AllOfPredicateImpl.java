package dev.wyck.wrapper.worldgen.blockpredicates;

import dev.wyck.wrapper.internal.Wrapper;
import org.jetbrains.annotations.ApiStatus;
import org.jspecify.annotations.NullMarked;

import java.util.List;

@NullMarked
@ApiStatus.Internal
public final class AllOfPredicateImpl extends CombiningPredicateImpl implements AllOfPredicate {

    public AllOfPredicateImpl(List<BlockPredicate> predicates) {
        super(predicates);
    }

    @Override
    public Object toMinecraft() {
        return net.minecraft.world.level.levelgen.blockpredicates.BlockPredicate.allOf(
            this.predicates.stream().map(Wrapper::<net.minecraft.world.level.levelgen.blockpredicates.BlockPredicate>asHandle).toList()
        );
    }
}
