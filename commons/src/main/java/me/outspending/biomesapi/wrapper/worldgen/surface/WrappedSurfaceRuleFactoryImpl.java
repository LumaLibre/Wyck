...
@WireFactory
@ApiStatus.Internal
public class WrappedSurfaceRuleFactoryImpl implements WrappedSurfaceRule.Factory {

    @Override
    public Object toNms(WrappedSurfaceRule rule) {
        return switch (rule) {
            case WrappedSurfaceRule.Bandlands ignored -> SurfaceRules.bandlands();
            case WrappedSurfaceRule.Block block -> block(block);
            case WrappedSurfaceRule.Sequence sequence -> sequence(sequence);
            case WrappedSurfaceRule.Condition condition -> condition(condition);
        };
    }

    private SurfaceRules.RuleSource block(WrappedSurfaceRule.Block block) {
    ...
    private SurfaceRules.RuleSource sequence(WrappedSurfaceRule.Sequence sequence) {
        List<SurfaceRules.RuleSource> sources = new ArrayList<>();
        for (WrappedSurfaceRule rule : sequence.rules()) {
    ...
    private SurfaceRules.RuleSource condition(WrappedSurfaceRule.Condition condition) {
