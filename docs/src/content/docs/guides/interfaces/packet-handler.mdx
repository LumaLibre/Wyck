...
To use the `PacketHandler`, your plugin must *depend* on **ProtocolLib** or **PacketEvents** (or softdepend both).
...
This can be changed with the [`force-per-user-registries` JVM flag](/guides/interfaces/packet-handler/#packetevents-caching-registries).
